...
path = 'F:\\Assignments\\Third_year\\second-semester\\Mobile Dev\\back-end\\main.db'
...
class User:
    ...
    @staticmethod
    def createUser(email:str , password:str , age:int = 18):
        query = User.model.insert().values(email=email , password=password , age=age)
        try:
...
class Match:
    ...
    keys = ["id" , "homeTeam" , "awayTeam" , "tournamentId" , "date" , "finished" , "elapsedTime"]

    @staticmethod
    def matchesForDate(date : int):
        query = Match.model.select().where(Match.model.c.date == date)
        ...
        try:
            ...
            for match in result:
                ...
                matchInfo['score'] = {
                    "home" : matchStat["homeStats"]["score"],
                    "away" : matchStat["awayStats"]["score"]
                }
...
class Team:
    ...
    def getLeagueTeams(leagueId : int):
        query = Team.model.select().where(Team.model.c.league_id == leagueId)
        try:
    ...
    def getTeamsOfUser(userId : int):
        ...
        try:
            teams = connection.execute(query)
            result = []
            for team in teams:
                temp = {
                        "teamId" : team[0],
